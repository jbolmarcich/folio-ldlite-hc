--This query returns a count of titles, or srs bib record uuids, and items, or item uuids, for all those physical monographs in a specific location.
...
AND 
	(lt.name like 'HC%' or  lt.code in ('FCANI', 'FCDPS', 'FCDPM')) --Looks FOR names that BEGIN WITH MH WHERE PERCENT sign IS the wildcard AND SPECIFIC codes
GROUP BY 
